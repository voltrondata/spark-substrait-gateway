...
class SparkSubstraitConverter:
        ...
        match literal.WhichOneof("literal_type"):
            case "null":
                ...
                result = algebra_pb2.Expression.Literal(
                    null=type_pb2.Type(
                        fp64=type_pb2.Type.FP64(nullability=type_pb2.Type.NULLABILITY_NULLABLE)
                    )
                )
            case "binary":
    ...
    def convert_when_function(
        ...
        if len(when.arguments) % 2 == 1:
            ...
            getattr(ifthen, "else").CopyFrom(
                algebra_pb2.Expression(
                    literal=algebra_pb2.Expression.Literal(null=nullable_literal)
                )
            )
    ...
    def convert_rlike_function(
        ...
        regexp_expr = regexp_strpos_function(
            regexp_strpos_func,
            self.convert_expression(in_.arguments[1]),
            self.convert_expression(in_.arguments[0]),
            bigint_literal(1),
    ...
    def convert_type_str(self, spark_type_str: str | None) -> type_pb2.Type:
        ...
        match spark_type_str:
            ...
            case _:
                raise NotImplementedError(f"type {spark_type_str} not yet implemented.")
        ...
        try:
            match expr.WhichOneof("expr_type"):
                ...
                case "expression_string":
                    raise NotImplementedError("SQL expressions through selectExpr is not supported")
                ...
                case "sort_order":
                    raise NotImplementedError("sort_order expression type not supported")
                ...
                case "update_fields":
                    raise NotImplementedError("update_fields expression type not supported")
    ...
    def convert_field(self, field: types_pb2.DataType) -> tuple[type_pb2.Type, list[str]]:
        ...
        match field.get("type"):
            ...
            case "decimal":
                field_type = type_pb2.Type(decimal=type_pb2.Type.Decimal(nullability=nullability))
            case "string":
            ...
            match str(field.type):
                ...
                case _:
                    if str(field.type).startswith("struct"):
                        ...
                        field_type = type_pb2.Type(
                            map=type_pb2.Type.Map(
                                nullability=nullability, key=key_type, value=value_type
                            )
                        )
                    elif str(field.type).startswith("decimal"):
                        _, precision, scale = _extract_decimal_parameters(str(field.type))
                        field_type = type_pb2.Type(
                            decimal=type_pb2.Type.Decimal(
                                nullability=nullability, precision=precision, scale=scale
                            )
                        )
                    else:
                        raise NotImplementedError(
    ...
    def convert_read_data_source_relation(self, rel: spark_relations_pb2.Read) -> algebra_pb2.Rel:
        ...
        paths = [str(path) for path in rel.paths] if rel.paths else list(rel.options.values())
        if not schema:
        ...
        if self._conversion_options.use_named_table_workaround:
            return algebra_pb2.Rel(
                read=algebra_pb2.ReadRel(
                    base_schema=schema,
                    named_table=algebra_pb2.ReadRel.NamedTable(names=["demotable"]),
                    common=self.create_common_relation(),
                )
            )
        ...
        result = algebra_pb2.Rel(
            read=algebra_pb2.ReadRel(
                base_schema=schema, local_files=local, common=self.create_common_relation()
            )
        )
        ...
        project = algebra_pb2.ProjectRel(
            input=result, common=algebra_pb2.RelCommon(direct=algebra_pb2.RelCommon.Direct())
        )
    ...
    def convert_sort_relation(self, rel: spark_relations_pb2.Sort) -> algebra_pb2.Rel:
        ...
        for order in rel.order:
            ...
            sort.sorts.append(
                algebra_pb2.SortField(
                    expr=self.convert_expression(order.child), direction=direction
                )
            )
    ...
    def convert_limit_relation(self, rel: spark_relations_pb2.Limit) -> algebra_pb2.Rel:
        ...
        self.update_field_references(rel.input.common.plan_id)
        fetch = algebra_pb2.FetchRel(
            common=self.create_common_relation(), input=input_relation, count=rel.limit
        )
    ...
    def convert_show_string_relation(self, rel: spark_relations_pb2.ShowString) -> algebra_pb2.Rel:
        ...
        project1 = project_relation(
            input_rel,
            [
                strlen(strlen_func, cast_operation(field_reference(column_number), string_type()))
        ...
        def full_line(fields: list[Field]) -> list[algebra_pb2.Expression]:
            return [
                concat(
                    concat_func,
                    flatten(
                        [field_line_fragment(field_number) for field_number in range(len(fields))]
                    )
                    + [
                        string_literal("+\n"),
                    ],
        ...
        aggregate2 = aggregate_relation(
            project4, measures=[string_concat_agg_function(string_concat_func, 0)]
        )
        ...
        project5 = project_relation(
            ...
            [
                concat(
                    ...
                    + [
                        string_literal(compute_row_count_footer(rel.num_rows))
                        if rel.num_rows
                        else None
                    ],
                ),
    ...
    def convert_arrow_to_literal(self, val: pa.Scalar) -> algebra_pb2.Expression.Literal:
        ...
        if isinstance(val, pa.BooleanScalar):
            ...
            raise NotImplementedError(f"Conversion from arrow type {val.type} not yet implemented.")
    ...
    def convert_cross_join_relation(self, rel: spark_relations_pb2.Join) -> algebra_pb2.Rel:
        ...
        join = algebra_pb2.CrossRel(
            left=self.convert_relation(rel.left), right=self.convert_relation(rel.right)
        )
    ...
    def convert_offset_relation(self, rel: spark_relations_pb2.Offset) -> algebra_pb2.Rel:
        ...
        fetch = algebra_pb2.FetchRel(input=input_rel, offset=rel.offset, count=rest_of_input)
        self.update_field_references(rel.input.common.plan_id)
    ...
    def convert_relation(self, rel: spark_relations_pb2.Relation) -> algebra_pb2.Rel:
        ...
        self._symbol_table.add_symbol(
            rel.common.plan_id, parent=self._current_plan_id, symbol_type=rel.WhichOneof("rel_type")
        )
    ...
    def convert_plan(self, plan: spark_pb2.Plan) -> plan_pb2.Plan:
        ...
        for uri in sorted(self._function_uris.items(), key=operator.itemgetter(1)):
            result.extension_uris.append(
                extensions_pb2.SimpleExtensionURI(extension_uri_anchor=uri[1], uri=uri[0])
            )
